...
export default function Home() {
  ...
  const discovery = useSpaceDiscovery(isHydrated && isLoggedIn, preferences.autoRefresh);
  ...
  return (
    <AppShell activeTab="EXPLORE" userName={userName} onSignOut={() => void handleSignOut()}>
      <SpaceDashboard discovery={discovery} />
    </AppShell>
  );
}
    ...
    <main className="min-h-dvh overflow-hidden bg-slate-950 px-5 py-6 text-white sm:px-8">
      <div className="mx-auto flex min-h-[calc(100dvh-3rem)] w-full max-w-7xl flex-col">
        ...
        <section className="grid flex-1 items-center gap-10 py-12 lg:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.7fr)]">
          ...
          <motion.div className="border-l border-slate-700 pl-6 sm:pl-8" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
